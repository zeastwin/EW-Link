...
- 功能：带进度的 AJAX 上传、弹窗创建文件夹、选中文件/文件夹批量 ZIP（目录递归打包，空目录占位）、批量删除（入回收站）、批量移动、文件/文件夹重命名、回收站浏览/批量还原/彻底删除、文件预览（文本/图片/音频/视频/PDF/ZIP 列表，20MB 内）、面包屑/返回、双击进入目录、客户端计数已选项。
