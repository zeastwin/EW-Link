...
- 上传限制 1GB，由 Kestrel 请求限制与 `ResourceStore` 双重校验；请求头超时时间 2 分钟。
- 数据保护密钥持久化至 `/Data/aspnet-keys`。

## 关键服务与安全措施
- `ResourceStore`（`Services/ResourceStore.cs`）：列目录、上传、删除、单文件读取；校验文件/文件夹名；同名冲突追加 ` (n)`；支持删除文件或目录；批量打包时可递归收集文件夹（含空目录占位），去重并阻止直接打包资源根；支持重命名、批量移动（同区内，阻止自包含移动与同名覆盖）。
...
- 功能：带进度的 AJAX 上传、弹窗创建文件夹、选中文件/文件夹批量 ZIP（目录递归打包，空目录占位）、批量删除、批量移动、文件/文件夹重命名、文件预览（文本/图片/音频/视频/PDF/ZIP 列表，20MB 内）、面包屑/返回、双击进入目录、客户端计数已选项。
